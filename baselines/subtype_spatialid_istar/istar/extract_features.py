...
def main_my(d, random_state=0, smoothen_method='cv', reduction_method=None, n_components=None, device='cuda:1'):
    
    np.random.seed(random_state)
    torch.manual_seed(random_state)

    # load data
    wsi = get_data(prefix=d)
    ...
    if reduction_method is not None:
        embs, reducers = reduce_embs_dim(
                embs, n_components=n_components,
                method=reduction_method, balance=False,
                groups=['cls', 'sub'])
        save_pickle(reducers, d+'reducers.pickle')

    save_embeddings(embs, d + 'embeddings-hist.pickle')
