...
def get_data(prefix, cnts):
    gene_names = read_lines(f'{prefix}gene-names.txt')
    cnts = get_gene_counts(cnts)
...
def main_my(d, cnts, epochs=400, n_states=5, load_saved=False, device='cuda', n_jobs=1):
    
    embs, cnts, locs = get_data(d, cnts)

    factor = 32 # 16 调整radius
    radius = int(read_string(f'{d}radius.txt'))
    radius = radius / factor
